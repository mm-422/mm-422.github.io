...
In order to create a better picture of where the vulnerability lies, a demonstration of a typical attestation workflow is set up to highlight all the "moving parts" at every step.

Let's assume a scenario where a client machine is requesting a new certificate from a Certificate Authority tied to a VPN server. The client would need to prove its identity and trustworthiness. This can be done by sending a part of its identity to the server in order to be verified.

But which part does the server accept? The answer is a legitimate key generated by a TPM. Keep in mind that the Endorsement Key is derived from a seed (Endorsement Primary Seed) which is burned at the factory. This means that all keys in this hierarchy are implicitly trusted.

However, the Certificate Authority will not accept a key generated by the TPM without any security guarantees. It is still possible to send a forged key to the server. This is where the process of attestation comes in, where the TPM generates data that in a way "vouches" for the system's state.

With the IBM TPM Simulator, this begins with the generation of an Endorsement Key:
```
# Initialization Step
$ powerup
$ startup -v

# Generate a parent key in the Endorsement Hierarchy
...
Once an EK is created and the Endorsement Hierarchy established, we can create new child keys.
