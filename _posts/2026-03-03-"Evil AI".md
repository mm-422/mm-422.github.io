...
- However, clever attacker can still hide behind seemingly harmless syntax.

### ♦️ DNS Filtering
- Block outbound DNS queries to newly registered domains.
- Restrict dev environment network interfaces.
- May not be practical depending on requirements, environment, and budget.
- Ex. Same workstation used for hosting local model is used for internet access.
...
- Might require a lot more oversight work which can lead to extra overhead.
