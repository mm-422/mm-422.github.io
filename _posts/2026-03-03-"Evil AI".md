...
A(Clean GitHub repo) -->|cloned to local env| B(LLM executes install script)
B --> C(Script throws error instructing re-run with additional flag)
C -->|Script re-run with flag| D(Shell spawned)
D -->|Queries malicious domain| E(Payload in DNS TXT obtained)
E -->|Config value added to shell| F(Connection to attacker machine established)
```
...
## Mitigation
- Tool Sandboxing.
- Granular Tool Permission Boudaries.
- Egress Network Filtering & DNS Sinkholing.
